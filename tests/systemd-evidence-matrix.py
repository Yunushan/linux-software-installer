...
def build_plan(root: Path) -> list[dict[str, str]]:
    ...
    if len(evidence_keys) != 13 or len({module for _, module in evidence_keys}) != 11:
        raise MatrixError("service-bearing readiness coverage drifted from 13 contracts / 11 modules")
    ...
    if len(rows) != 58 or len(execution_ids) != len(set(execution_ids)):
        raise MatrixError("systemd evidence plan must contain 58 unique executions")
    return rows
