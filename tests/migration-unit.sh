...
test_canonical_catalog_loads() (
  ...
  [[ $LSI_MIGRATION_TOTAL -eq 355 &&
    $LSI_MIGRATION_TERMINAL -eq 189 &&
    $LSI_MIGRATION_PLANNED -eq 40 &&
...
test_list_is_complete_and_nonclaiming() (
  ...
  [[ $count -eq 355 ]] &&
    grep -q '355 entries: 189 terminal, 40 provisional, 126 unresolved third-party' <<< "$output" &&
...
test_retirement_status_reports_exact_blockers() (
  ...
  grep -q '^Tracked legacy entries        : 355$' <<< "$output" &&
    grep -q '^Terminal dispositions         : 189$' <<< "$output" &&
    grep -q '^Provisional module candidates : 40$' <<< "$output" &&
    grep -q '^Unresolved third-party routes : 126$' <<< "$output" &&
    grep -q '^Accepted evidence admissions  : 77$' <<< "$output" &&
...
test_admitted_terminal_replacement_loads() (
  ...
  lsi_migration_load || return 1
  output=$(lsi_migration_show ubuntu-002) || return 1
  [[ $LSI_MIGRATION_TERMINAL -eq 190 && $LSI_MIGRATION_PLANNED -eq 39 ]] &&
