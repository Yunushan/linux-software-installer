...
test_canonical_catalog_loads() (
  ...
  [[ $LSI_MIGRATION_TOTAL -eq 355 &&
    $LSI_MIGRATION_TERMINAL -eq 216 &&
    $LSI_MIGRATION_PLANNED -eq 37 &&
    $LSI_MIGRATION_BLOCKED -eq 102 ]]
)
...
test_list_is_complete_and_nonclaiming() (
  ...
  [[ $count -eq 355 ]] &&
    grep -q '355 entries: 216 terminal, 37 provisional, 102 unresolved third-party' <<< "$output" &&
...
test_admitted_terminal_replacement_loads() (
  ...
  [[ $LSI_MIGRATION_TERMINAL -eq 216 && $LSI_MIGRATION_PLANNED -eq 37 ]] &&
