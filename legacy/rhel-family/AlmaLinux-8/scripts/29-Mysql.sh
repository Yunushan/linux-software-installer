...
if [ "$mysqlversion" = "1" ];then
    ...
    sudo dnf -vy module reset mysql -y && sudo dnf module disable mysql
...
gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-mysql" > /etc/yum.repos.d/mysql-community.repo
    ...
    | grep -i rpm | head -n 1)
    mysql_community_icu_data_files=$(lynx -dump https://dev.mysql.com/downloads/file/?id=509907 \
    | awk '/http/{print $2}' | grep -i rpm | head -n 1)
