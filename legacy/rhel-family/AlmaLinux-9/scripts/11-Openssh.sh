...
if [ "$opensshversion" = "1" ];then
    ...
    latest_openssh_name=$(lynx -dump https://www.openssh.com/releasenotes.html | awk '/http/{print $2}' | grep -i p1.tar.gz | head -n 1 \
    | sed 's/.......$//' | cut -c 54-70)
    sudo dnf -vy install gcc zlib zlib-devel compat-openssl11 openssl openssl-devel zlib-devel openssl-devel pam-devel \
    libselinux-devel audit-libs-devel autoconf automake gcc libX11-devel libselinux-devel make ncurses-devel \
    openssl-devel p11-kit-devel perl-generators systemd-devel xauth pam-devel rpm-build zlib-devel \
    rpm-build rpmdevtools rpmlint gtk2-devel imake libXt-devel openssl-devel perl gtk2-devel libX11-devel \
    imake krb5-devel libXt-devel
    rpmdev-setuptree
    sudo dnf -vy group install 'Development Tools'
    #opensshlatest=$(lynx -dump https://www.openssh.com/releasenotes.html | awk '/http/{print $2}' \
    #| grep -i p1.tar.gz | head -n 1)
    sudo mkdir -pv /root/rpmbuild/SOURCES/"$latest_openssh_name"
    sudo mkdir -pv /root/rpmbuild/SPECS
    wget -O /root/rpmbuild/SOURCES/"$latest_openssh_name".tar.gz \
    https://cdn.openbsd.org/pub/OpenBSD/OpenSSH/portable/"$latest_openssh_name".tar.gz #"$opensshlatest"
    ...
    tar -xvf /root/rpmbuild/SOURCES/"$latest_openssh_name".tar.gz -C /root/rpmbuild/SOURCES/"$latest_openssh_name" --strip-components 1
    sudo cp -v /root/rpmbuild/SOURCES/"$latest_openssh_name"/contrib/redhat/openssh.spec /root/rpmbuild/SPECS/openssh.spec
    ...
    sudo dnf -vy remove openssh openssh-clients openssh-server
    sudo dnf -vy install /root/rpmbuild/RPMS/x86_64/"$latest_openssh_name"-1.el9.x86_64.rpm
    sudo dnf -vy install /root/rpmbuild/RPMS/x86_64/"$latest_openssh_name"-1.el9.x86_64.rpm
    sudo dnf -vy install /root/rpmbuild/RPMS/x86_64/"$latest_openssh_name"-1.el9.x86_64.rpm
