...
sudo dnf -vy install elrepo-release

if [ "$kernelversion" = "1" ];then
    sudo dnf -vy --enablerepo=elrepo-kernel install kernel-ml kernel-ml-devel kernel-ml-headers
elif [ "$kernelversion" = "2" ];then
    sudo dnf -vy --enablerepo=elrepo-kernel install kernel-lt kernel-lt-devel kernel-lt-headers
else
