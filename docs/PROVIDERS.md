...
foundation** with narrowly scoped APT and DNF transaction boundaries. It can validate
...
supports exact locked APT and DNF package transactions; both remain unavailable
to real users until a provider is independently reviewed and registered. No live
...
installs that verified local artifact. The equivalent DNF transaction enables
only the reviewed provider repository while downloading the exact locked RPM,
requires signed repository metadata and packages, verifies the RPM's SHA-256
and NEVRA, then enforces either its exact signed `Vendor` origin or an exact
`signer:<full-primary-fingerprint>` origin. The signer form is valid only when
the target cell declares that same single primary key, and the RPM verifies in
an isolated RPM database containing only the checked-in key; it cannot be an
empty-Vendor exception. The transaction then installs the verified local RPM
with that provider repository disabled. Both paths invoke the declared binary with
`--version` and remove repository files again unless `--persist-provider` was
explicitly accepted.
...
Before a provider can become live, its APT or DNF route must have the
transaction boundary above plus real target-cell evidence. Every provider
still must prove:
...
- exact package-version, package identity, digest and origin enforcement with
  fail-closed solver checks (the DNF `expected_origin` is either the signed
  RPM `Vendor` field, or `signer:<full-primary-fingerprint>` that exactly
  matches the cell's sole declared primary key; the locked RPM may be
  downloaded only from the activated provider repository);
...
The 102 unresolved third-party legacy rows and their recommended provider or
