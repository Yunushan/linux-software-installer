...
The current inventory snapshot has 186 terminal rows: 101 `implemented`, one
`superseded`, 20 `blocked-safety`, 25 `retired` and 39 `out-of-scope`. The
remaining 169 rows are non-terminal: 43 `planned` active-module candidates and
...
module; 102 active-replacement rows now carry accepted evidence and completed
...
- 10 planned Ubuntu rows with an explicit Debian-family active-module candidate
...
- 43 candidate rows in total and 126 provider/repository rows in total, all
...
externally hashed aggregate admitted 74 module-family keys and 102 active rows,
including the Steam multiarch contract and twenty-nine additional non-service
...
145 rows: 101 currently `implemented`, one `superseded` and 43 still `planned`.
...
The current ledger has 102 Debian-family and 72 RHEL-family active-replacement
rows. One hundred two rows have accepted evidence and promotion readiness `yes`;
43 remain at evidence class `none` and readiness `no`. Their mappings collapse
...
[`accepted-evidence.tsv`](accepted-evidence.tsv) currently contains 74
accepted module-family admissions, covering 102 active ledger and terminal
inventory rows. Each
...
The smallest safe path to close the remaining 43 planned rows is:

1. Publish repository-resolution evidence for the tested commit, exact image
   digests and all 19 remaining module-family contracts (or a reviewed stronger
