...
The current inventory snapshot has 181 terminal rows: 96 `implemented`, one
`superseded`, 20 `blocked-safety`, 25 `retired` and 39 `out-of-scope`. The
remaining 174 rows are non-terminal: 48 `planned` active-module candidates and
...
module; 97 active-replacement rows now carry accepted evidence and completed
...
- 15 planned Ubuntu rows with an explicit Debian-family active-module candidate
...
- 48 candidate rows in total and 126 provider/repository rows in total, all
...
externally hashed aggregate admitted 69 module-family keys and 97 active rows,
including the Steam multiarch contract and twenty-four additional non-service
...
145 rows: 96 currently `implemented`, one `superseded` and 48 still `planned`.
...
The current ledger has 97 Debian-family and 72 RHEL-family active-replacement
rows. Ninety-seven rows have accepted evidence and promotion readiness `yes`;
48 remain at evidence class `none` and readiness `no`. Their mappings collapse
...
[`accepted-evidence.tsv`](accepted-evidence.tsv) currently contains 69
accepted module-family admissions, covering 97 active ledger and terminal
inventory rows. Each
...
The smallest safe path to close the remaining 48 planned rows is:

1. Publish repository-resolution evidence for the tested commit, exact image
   digests and all 24 remaining module-family contracts (or a reviewed stronger
