...
The current inventory snapshot has 176 terminal rows: 91 `implemented`, one
`superseded`, 20 `blocked-safety`, 25 `retired` and 39 `out-of-scope`. The
remaining 179 rows are non-terminal: 53 `planned` active-module candidates and
...
module; 92 active-replacement rows now carry accepted evidence and completed
...
- 20 planned Ubuntu rows with an explicit Debian-family active-module candidate
...
- 53 candidate rows in total and 126 provider/repository rows in total, all
...
externally hashed aggregate admitted 64 module-family keys and 92 active rows,
including the Steam multiarch contract and nineteen additional non-service
...
145 rows: 91 currently `implemented`, one `superseded` and 53 still `planned`.
...
The current ledger has 92 Debian-family and 72 RHEL-family active-replacement
rows. Ninety-two rows have accepted evidence and promotion readiness `yes`;
53 remain at evidence class `none` and readiness `no`. Their mappings collapse
...
[`accepted-evidence.tsv`](accepted-evidence.tsv) currently contains 64
accepted module-family admissions, covering 92 active ledger and terminal
inventory rows. Each
...
The smallest safe path to close the remaining 53 planned rows is:

1. Publish repository-resolution evidence for the tested commit, exact image
   digests and all 29 remaining module-family contracts (or a reviewed stronger
