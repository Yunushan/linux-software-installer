...
| `distro-component` | `implement` | 2 | Verify maintained Debian-family packages on every claimed image. |
...
and 79 canonical outcomes. Of those rows, 47 point to 20 existing modules:
`clamav`, `composer`, `deluge`, `dvblast`, `fail2ban`, `ffmpeg`, `htop`,
`magic-wormhole`, `makehuman`, `monitoring-tools`, `neovim`, `quassel`, `timeshift`,
...
12-only. MakeHuman is a Debian-family package-only candidate with no accepted
evidence. Tor Browser Launcher remains one Debian distro-component candidate
restricted to one exact Tier-1 cell. Therefore the number of family-wide
module-contract reuse rows is **one**, with one target-restricted reuse row;
neither is promotion-ready. A row remains blocked until its applicable target
coverage and durable solver/install/repeat-install evidence are accepted. The
other 79 rows cover 59 outcomes for which no active module exists yet.
...
| --- | ---: | ---: | ---: | --- |
| Distribution components | 2 | 2 | 2 rows / 2 outcomes | MakeHuman needs all claimed Debian-family solver/install/repeat evidence; Tor Browser needs first-run browser evidence. |
