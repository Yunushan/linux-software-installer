...
It is a planning ledger, not replacement evidence: all 102 source inventory rows
...
The dated [`RPM_FUSION_AUDIT.md`](RPM_FUSION_AUDIT.md) reaches the same
metadata-trust boundary for all ten `rpm-fusion` rows: the official EL 9 Free
repository key and `repomd.xml` are available, but its detached
`repomd.xml.asc` endpoint returned HTTP 404. The current DNF policy therefore
correctly keeps this provider unregistered.
...
The dated [`JENKINS_PROVIDER_AUDIT.md`](JENKINS_PROVIDER_AUDIT.md) independently
verified the current Jenkins Debian and RPM metadata signatures against the
published 2026 Jenkins signing key. It identifies viable planning candidates
for both package families: the RHEL RPM's empty `Vendor` can use the reviewed
exact-signer contract, but no provider is registered. Every Jenkins row stays
blocked until its applicable provider design, exact locks, module/service
contracts and accepted target evidence exist.

The dated [`ANYDESK_PROVIDER_AUDIT.md`](ANYDESK_PROVIDER_AUDIT.md) verifies the
current AnyDesk Debian and RPM metadata signatures against the vendor-published
key. Its Debian route is a future candidate only; the vendor's current Linux
support policy names RHEL 8 but not the active EL 9 targets, so the historical
CentOS 7 RPM row has no current promotion route.

The dated [`SNAP_PROVIDER_AUDIT.md`](SNAP_PROVIDER_AUDIT.md) separates the 13
Ubuntu Snap Store candidates from the 12 historical or EPEL-dependent
RHEL-family rows. It requires assertion validation, exact publisher/channel/
confinement review, explicit classic acknowledgement, and fresh-VM service
evidence before any future Snap route can be admitted.

The dated [`4KDOWNLOAD_PROVIDER_AUDIT.md`](4KDOWNLOAD_PROVIDER_AUDIT.md)
separates current vendor facts from automation evidence for the four 4K Download
artifact rows. Their free tiers and user-owned license or service-login choices
do not authenticate a mutable download; all four remain conditional artifacts
until exact vendor verification and maintained-target evidence exist.

The dated [`PUBLIC_ARTIFACT_PROVIDER_AUDIT.md`](PUBLIC_ARTIFACT_PROVIDER_AUDIT.md)
lists all fifteen current public-artifact rows, including their source-specific
verification and safety boundaries. It records required work only: no artifact
provider is registered and no row is promoted by that audit.

The dated [`VENDOR_REPOSITORY_PROVIDER_AUDIT.md`](VENDOR_REPOSITORY_PROVIDER_AUDIT.md)
captures the five remaining vendor-APT routes not already covered by the
product-specific audits. It requires a scoped keyring, exact package lock and
accepted runtime evidence before any provider can be admitted.

Run `bash tests/validate-provider-backlog.sh` and
`bash tests/validate-provider-audit-coverage.sh` after changing either ledger.
The first validator requires an exact one-to-one join by `legacy_id`, rejects
duplicate or extra rows, checks capability identity, and freezes both the action
and strategy totals below. The second validator fails closed if any unresolved
route has no named provider-audit boundary. To inspect the derived one-to-one
mapping without editing a second source of truth, run:
...
| --- | ---: | --- |
| `implement` | 87 | Build and verify a provider/module candidate. |
| `conditional-artifact` | 15 | Automate only when immutable, authenticated upstream artifacts can be proved. |
| `terminal-handoff` | 0 | No reviewed handoff remains unresolved in the active backlog. |
| **Total** | **102** | Exact current `blocked-third-party` coverage. |
...
The rows previously recommended for terminal review now have durable official
evidence in [`LEGACY_DISPOSITIONS.md`](LEGACY_DISPOSITIONS.md). Those terminal
rows are intentionally
...
| `distro-component` | `implement` | 0 | No unresolved route currently uses a maintained distribution component. |
...
| `public-artifact` | `conditional-artifact` | 15 | Require an immutable version plus a trusted upstream signature or digest; otherwise hand off. |
...
| `snap-store` | `implement` | 23 | Review publisher, channel, confinement, and repeat-install behavior before admitting a snap. |
| `vendor-apt` | `implement` | 9 | Use a scoped keyring and verify signed repository, solver, install, and update behavior. |
| `vendor-rpm` | `implement` | 8 | Pin the vendor key identity and verify signed repository, solver, install, and update behavior. |

## Machine-checked closure shape

The validator also cross-checks every proposed `replacement_outcome` against
the active module catalog. The 102 rows represent 61 normalized capabilities
and 59 canonical outcomes. Of those rows, 45 point to 18 existing modules:
...
other 57 rows cover 41 outcomes for which no active module exists yet.

MakeHuman has no maintained distribution-package route on the reviewed images:
its current upstream guidance is a PPA or source build, so it is a conditional
external-artifact route. PlayOnLinux, Telegram, Steam, and Tor Browser are
accepted on their exact verified target cells. The probe history is recorded in
[`DISTRO_COMPONENT_PROBES.md`](DISTRO_COMPONENT_PROBES.md). All 102 current
rows require an external signed repository/store or an immutable, authenticated
...
| Vendor APT and RPM | 17 | 9 | 0 rows / 0 outcomes | Family-specific repository adapters and new modules; four outcomes span both families. |
| Snap bootstrap and store | 25 | 17 | 0 rows / 0 outcomes | Resolve the EPEL-dependent bootstrap trust blocker before designing an opt-in snap subsystem, application outcomes and service/confinement evidence. |
| Public artifacts | 15 | 15 | 0 rows / 0 outcomes | One fail-closed version/signature-or-digest contract per artifact. |
...
59 unique outcomes, including the Snap provider bootstrap outcome.

This gives a concrete closure order: resolve the EPEL trust-model blocker before
any EPEL implementation, investigate
